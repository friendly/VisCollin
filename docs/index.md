...
shows the coefficients, $`\hat{\beta_j}`$, their standard errors
$`s(\hat{\beta_j})`$ and associated $`t`$ statistics,
...
Or, better yet, use
[`corrplot::corrplot.mixed()`](https://rdrr.io/pkg/corrplot/man/corrplot.mixed.html)
...
We check the variance inflation factors, using
[`car::vif()`](https://rdrr.io/pkg/car/man/vif.html). We see that most
predictors have very high VIFs, indicating moderately severe
multicollinearity.
...
- **Condition indices**: The smallest of the eigenvalues, those for
  ...
  relation to the largest. This leads to *condition indices*, defined as
...
[`colldiag()`](https://friendly.github.io/VisCollin/reference/colldiag.md)
printed above (but using `prop.col` color breaks such that variance
proportions \< 0.3 are shaded white). The visual message is that one
should attend to collinearities with large condition indices **and**
...
![](reference/figures/README-cars-biplot-1.png)
